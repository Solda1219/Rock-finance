...
library AMPOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IAMPPair(pair).price0CumulativeLast();
        price1Cumulative = IAMPPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IAMPPair(pair).getReserves();
...
contract RewardPool is Ownable{
    ...
    modifier onlyRouter() {
        require(msg.sender == router, "RewardPool: caller is not the router");
        _;
    }
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(IAMPPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function permit(address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) private {
        require(deadline >= block.timestamp, 'RewardPool: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == spender, 'RewardPool: INVALID_SIGNATURE');
    }

    function withdraw(uint8 v, bytes32 r, bytes32 s, uint deadline) public returns(bool){
        require(maxMiningAmount > totalMined, 'RewardPool: Mined all tokens');
        uint256 balance = _balances[msg.sender];
        require(totalMined.add(balance) <= currentPhase.mul(maxMiningInPhase), 'RewardPool: Mined all tokens in this phase');
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IAMPFactory(factory).getPair(outputToken, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IAMPFactory(factory).getPair(outputToken, intermediate) != address(0) 
                    && IAMPFactory(factory).getPair(intermediate, anchorToken) != address(0)) {
